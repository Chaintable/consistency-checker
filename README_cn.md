...
进度存入现有 `consistency_db_path` 的 Pebble 数据库，元数据 key 为 `meta/fork-recheck/v1/<chainID>/<十六进制编码的version>`。每批最多扫描 128 个高度，执行预算为 10 秒；批末复核 canonical 和 reorg 影响范围，只将已成功修复的连续前缀同步落盘推进 `next_scan_height`。读取 canonical 和空 plan 扫描均不持主处理锁，只在批次启动、PUT 复核和批末落盘时持锁。游标不会越过失败高度。reorg（包括切到更短链）会在同一个批次更新 canonical 索引并回退游标，即使 hash 改走后又改回，旧扫描任务也不能覆盖回退结果。reorg 只从最早受影响高度截断已成熟和待成熟的观察，保留更低高度的成熟积压；替代分支上的高度重新等待观察周期。过期批次立即重新评估仍有效的前缀，不计入扫描错误。崩溃允许重复尚未落盘的批次，不会先记录完成再修复对象。
...
- 启动 outer 尾部为 `IsFork=true` 时，视为尚未完成的 reorg，不把 drop 当作扫描锚点或空 topic。启动保留原进度和 pending 状态，允许更早的重复消息通过，但不使巡检对齐；替代分支须通过现有连续性校验并完成发布后才能恢复巡检。若本地 DB 已重建，只能以完整发布的替代链头建立新基线。
...
启用连续模式后应观察 `fork_scan_backlog` 的走势，确认扫描吞吐跟得上目标链。连续游标保证覆盖范围内的每个高度接受一次延迟检查。如果对象在该高度扫描成功后才上传或被覆盖（例如几天后），仍可能无法发现；连续模式不做无限历史复查。已知 reorg 可以回扫到初始基线以下的高度。`fork_scan_interval_sec <= 0` 关闭所有巡检模式，显式配置 `fork_scan_lookback: 0` 也会关闭巡检。新版本兼容旧配置；旧二进制不能解析 `-1`，回滚镜像时须同时改回正值。
