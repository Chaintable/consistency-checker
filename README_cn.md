...
连续模式观察 checker 已完整发布的链头，等待一个完整巡检周期后，以上一次观察的高度为补扫上限。延迟按实际观察时间计算，不使用区块时间戳，也不额外运行最近 N 块扫描。慢扫描、积压的定时事件只会延长等待，不会让新观察的高度提前成熟。单个后台任务分批追赶积压；LIST/GET 在主处理锁外执行，持锁的每次 PUT 最多等待一秒。失败后保留下一个未完成高度，后续重试。
...
- 正常重启恢复游标，等待现有 inner/outer 连续性校验对齐，并重新等待观察周期。canonical 已写入但通知尚未完整发布时，进度保持 pending，直到通知处理或重放成功。
